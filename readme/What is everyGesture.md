...
| `ToggleAction` | `This` | Action toggle state helper instance. | `ToggleAction()` |
...
| `orientationReceiver` | `BroadcastReceiver` | Receiver used for configuration change events. | `createOrientationReceiver(handles)` |
...
| `lastGesture` | `Gesture` | Latest Gesture recognized. | `null` |
| `thisManager` | `This` | Manager used to load helper scripts into the runtime. | `null` |
| `packageManager` | `This` | Package manager instance. | `PackageManager()` |
| `updateManager` | `This` | Update manager instance created by [`UpdateManager`](/code/lib/UpdateManager.bsh). | `UpdateManager()` |
| `userService` | `This` | User service manager instance. | `UserServiceManager()` |
| `helperModeManager` | `This` | Helper mode manager instance. | `HelperModeManager()` |
| `uiReader` | `This` | UI reader manager instance. | `UiReader()` |
...
1. [`set()`](/code/gesture.java)
Dynamically import library commands into the interpreter classpath.
2. [`openMainActivity()`](/code/gesture.java)
Launch the main UI activity.
3. [`update()`](/code/gesture.java)
Run the update manager, then reload the runtime.
4. [`updatePreRelease()`](/code/gesture.java)
Run the pre-release update flow and reload the runtime.
5. [`reload()`](/code/gesture.java)
Reload and source the main `gesture.java` script.
6. [`cleanResources()`](/code/gesture.java)
Shut down executors, unregister receivers, and remove all handles to release resources.
7. [`inspectVariables(This targetThis)`](/code/gesture.java)
Open the variable inspector dialog for the specified `This` context.
8. [`inspectVariables(NameSpace targetNameSpace)`](/code/gesture.java)
Open the variable inspector dialog for the specified `NameSpace`.
9. [`inspectVariables(JavaCodeSceneV2Helper targetTasker)`](/code/gesture.java)
Open the variable inspector dialog for a task.
